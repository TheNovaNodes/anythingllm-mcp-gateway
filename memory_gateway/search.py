...
Contract: raw data only. No /chat, no LLM syntheses.
Both layers are executed in parallel; results are merged using Reciprocal Rank
...
# FTS5 snippet highlight markers (〈b〉…〈/b〉). Removed when extracting the anchor.
SNIP_OPEN = "\u27e8b\u27e9"
SNIP_CLOSE = "\u27e8/b\u27e9"

# AnythingLLM (utils/TextSplitter/index.js:145) wraps each chunk in
# <document_metadata>...</document_metadata> and adds the e5 prefix
# passage:/query: during embedding. This is a server wrapper - in the input of sync.py
# it is absent, so we cut it out on the gateway output side, so the agent receives
# clean text without internal XML-like garbage.
...
def _clean_text(text: str) -> str:
    """Removes AnythingLLM server wrappers: metadata block + e5 prefix."""
    if not text:
        return text
...
# ── Secret: cache the token in memory, read once ────────────────────────
...
def load_token() -> str:
    ...
    with _token_lock:
        ...
        fallback_token = os.environ.get("ANYTHINGLLM_API_KEY") or os.environ.get("MG_AUTH_TOKEN")

        if not path:
            if fallback_token:
                _token_cache = fallback_token.strip()
                return _token_cache
            raise RuntimeError("MG_TOKEN_FILE environment variable is not configured and no fallback env vars provided")

        if not os.path.exists(path):
            if fallback_token:
                log.warning(f"token file not found: {path}, falling back to env vars")
                _token_cache = fallback_token.strip()
                return _token_cache
            raise RuntimeError(f"token file not found: {path}")

        # Soft permissions check: secret must not be world-readable.
        try:
        ...
        if not tok:
            if fallback_token:
                log.warning(f"token file is empty: {path}, falling back to env vars")
                _token_cache = fallback_token.strip()
                return _token_cache
            raise RuntimeError(f"token file empty and no fallback env vars provided: {path}")
        _token_cache = tok
        return tok
            ...
            with open(config.MAP_FILE, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    if not line or line.startswith("#"):
                        continue
                    # format "src_slug real_slug" - we take the REAL one (2nd field).
                    parts = line.split()
...
def _doc_id_from_meta(meta: Dict[str, Any], title: str) -> str:
    ...
    Priority: relative path (if any) -> title/basename."""
...
# The file incremental_report.json is overwritten every 5 mins (alm-sync-incremental.service).
# On mtime change - full cache reset (garbage window <= mtime-latency).
_SYNC_STATE_FILE = os.environ.get(
    "MG_SYNC_STATE_FILE",
    os.path.join(config.OPS_DIR, "incremental_report.json"),
)
_SYNC_STATE_MTIME: float = 0.0
_SYNC_STATE_CHECK_INTERVAL = 5.0  # check mtime no more than once per 5 seconds
_SYNC_STATE_LAST_CHECK: float = 0.0


def _check_sync_state() -> None:
    """D2: cache reset on ALM index update.
    Checks mtime of incremental_report.json (written every 5 mins incrementally).
    Called before each search, but no more frequently than _SYNC_STATE_CHECK_INTERVAL.
    """
    ...
    if _SYNC_STATE_MTIME == 0.0:
        # first call - remember mtime without reset
...
def vector_search(query: str, top_k: int, threshold: float,
    ...
    The result is cached for MG_VECTOR_CACHE_TTL sec (see _vector_cache_*)."""
...
def _lexical_connect() -> sqlite3.Connection:
    """Read-only connection to lexical.db (write/corruption protection)."""
...
def lexical_search(query: str, top_k: int) -> List[Dict[str, Any]]:
    """FTS5/BM25 full-text search. OR by tokens (recall), BM25 ranks."""
    ...
    for path, title, rank, snip in rows:
        ...
        out.append({
            ...
            "workspace": None,
            "title": title,
            "doc_id": path,               # rel path - canonical id for get_document
            "text": clean,
...
def _dedup_key(item: Dict[str, Any]) -> str:
    """Deduplication key: document base name, case-insensitive.
...
def _minmax(vals: List[float]) -> Dict[float, float]:
    """Min-max list normalization to 0..1."""
    if not vals:
        return {}
...
def _fuse_weighted(vector_hits, lexical_hits, top_k):
    ...
    applies Temporal Decay to prefer fresh files.
    """
...
def rrf_merge(vector_hits: List[Dict[str, Any]],
    ...
    """RRF: score = sum(1/(rank+K)) across lists. Dedup by document name.
    On duplicate, keep the most informative variant (with non-empty text/workspace)."""
    k = config.RRF_K
    k = config.RRF_K
...
def hybrid_search(query: str, top_k: int,
    ...
    """Hybrid: vector + lexical in PARALLEL, merged. Returns clean JSON.

    degraded=True if one layer is unavailable (the other still returns data).
...
def store_memory(content: str, title: Optional[str] = None, workspace: Optional[str] = "dmagybot",
    ...
    """Active memory write tool (Issue #7). Uploads raw text to AnythingLLM and adds embeddings."""
...
# ── get_document: full raw text of the document ───────────────────────
...
def get_document(doc_id: str, max_chars: int = 20000) -> Dict[str, Any]:
    """Full raw text of a document by doc_id.

    Strategy (raw retrieval, no LLM):
    1) lexical.db (full content) by exact path, then by basename;
    2) fallback - metadata from AnythingLLM official API (/document/:name).
    """
...
# ── Context Assembly: expanding passage to a cohesive block ───────────
def _strip_anchor(text: str) -> str:
    """Removes FTS5 markers and collapses spaces for reliable anchor search."""
    return re.sub(r"\s+", " ", (text or "").replace(SNIP_OPEN, "").replace(SNIP_CLOSE, "").replace("\u2026", " ")).strip()


def _norm_map(text: str) -> tuple:
    """Normalizes text (lower + removing quotes + collapsing whitespace into
    a single space) for anchor search and returns (norm_text, offsets), where
    offsets[i] is the position of the i-th character of norm_text in the ORIGINAL text. Allows
    searching for seed without quotes/newlines, but get the correct position
...
def _lexical_fts_phrase(phrase: str) -> List[tuple]:
    """[(path, content)] by exact FTS5 phrase - exact match in the target document."""
...
def _lexical_fts_tokens(q: str) -> List[tuple]:
    ...
    Independent of document path - finds target doc by content.
    """
...
def _expand_result(result: Dict[str, Any]) -> None:
    """Mutates result: expands result['text'] with neighboring paragraphs of the same
    document. Ambiguity of names (SKILL.md, README.md) is resolved via
    FTS5 anchor phrase - selects the document where the anchor actually exists.
    ...
    # 2) fallback: FTS5 by anchor key tokens (AND) - independent of path.
    #    Punctuation (including ':' which breaks MATCH as column-filter) is stripped.
    if not candidates:
    ...
    if not candidates:
        return
    # select document containing the anchor (resolves ambiguity).
    # Verify via normalization (whitespace + quotes), so the anchor
    # is found even with newline differences.
    content = None
